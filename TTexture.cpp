...
void CTexture::createFromData(BYTE* bData, int a_iWidth, int a_iHeight, int a_iBPP, GLenum format, bool bGenerateMipMaps)
{
	...
	if (this->textureHint == GL_TEXTURE_CUBE_MAP)
	{
		if (format == GL_RGBA || format == GL_BGRA)
			glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + cubeMapIndex, 0, GL_RGBA, a_iWidth, a_iHeight, 0, format, GL_UNSIGNED_BYTE, bData);
		// We must handle this because of internal format parameter
		else if (format == GL_RGB || format == GL_BGR)
			glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + cubeMapIndex, 0, GL_RGB, a_iWidth, a_iHeight, 0, format, GL_UNSIGNED_BYTE, bData);
		else
			glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + cubeMapIndex, 0, format, a_iWidth, a_iHeight, 0, format, GL_UNSIGNED_BYTE, bData);
...
void CTexture::CreateDepthTexture(int a_iWidth, int a_iHeight)
{
	...
	glTexImage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, 0, GL_DEPTH_COMPONENT24, a_iWidth, a_iHeight, 0);
}
...
bool CTexture::loadTexture2D(std::string a_sPath, bool bGenerateMipMaps)
{
	...
	Logger::Log("Loaded texture" + a_sPath);
	return true; // Success
}


BYTE* CTexture::getDataPointer()
{
	BYTE* db = new BYTE[sizeof(bDataPointer)];
	db = bDataPointer;
	return db;
}

bool CTexture::LoadCubeMap(std::vector<std::string> tfile)
{
	
	isCubeMap = true;
	glGenTextures(1, &uiSampler);
	...
	for (int i = 0; i < tfile.size(); i++)
	{
		...
		fif = FreeImage_GetFileType(tfile[i].c_str(), 0); // Check the file signature and deduce its format

		if (fif == FIF_UNKNOWN) // If still unknown, try to guess the file format from the file extension
			fif = FreeImage_GetFIFFromFilename(tfile[i].c_str());

		if (fif == FIF_UNKNOWN) // If still unknown, return failure
			return false;

		if (FreeImage_FIFSupportsReading(fif)) // Check if the plugin has reading capabilities and load the file
			dib = FreeImage_Load(fif, tfile[i].c_str());
		if (!dib)
			return false;
		...
		if (FreeImage_GetBPP(dib) == 8)format = GL_LUMINANCE;
		if (format != 0)
			createFromData(bDataPointer, FreeImage_GetWidth(dib), FreeImage_GetHeight(dib), FreeImage_GetBPP(dib), format, true);
	...
	Logger::Log("Mipmaps generated");


	Logger::Log("Texture cube map loaded: ");
	return true;
}
...
void CTexture::bindTexture(int iTextureUnit)
{
		...
		glBindTexture(textureHint, uiTexture);
