...
void CShader::SetWindow(HWND hWnd)
{
	mainwin = hWnd;
}
...
bool CShader::loadShader(std::string sFile, int a_iType)
{
	...
	if (iCompilationStatus == GL_FALSE)
	{
		...
		{
			std::wstring stemp = s2ws(vectorerror);
			std::wstring scapt = s2ws(sFile);
			MessageBox(mainwin, (LPCWSTR)stemp.c_str(), (LPCWSTR)scapt.c_str(), MB_OK);
